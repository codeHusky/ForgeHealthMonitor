...
final class RemappingUtil {
    ...
    private static final Pattern METHOD_PATTERN = Pattern.compile("m_(\\d+)_"); // fabric = method_####  forge = m_#####_
    private static final Pattern FIELD_PATTERN = Pattern.compile("f_(\\d+)_"); // fabric = field_####   forge = f_#####_
    ...
    public static String remapString(CachedMappings mappings, String string) {
        ...
        if (string.contains("m_")) {
            string = remapMethods(mappings, string);
        }

        if (string.contains("f_")) {
